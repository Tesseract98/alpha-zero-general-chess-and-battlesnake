...
class BoardScreen(Screen):
    ...
    def on_touch_down(self, touch):
        res = super(BoardScreen, self).on_touch_down(touch)
    ...
    def on_touch_up(self, touch):
        ...
        res = super(BoardScreen, self).on_touch_up(touch)
        if res:
            ...
            if self.turn % 2 != 0:
                ...
                if is_move_valid == True:
                    ...
                    if not_a_promotion == True:
                        ...
                        self.ids[self.piece_that_moved].pos = (conversion.to_number()[self.chess_position_numerical][0],
                                                               conversion.to_number()[self.chess_position_numerical][1])
                        ...
                        # Adds the buttons the the layout that were just defined based on the color of the promoted
                        # piece
                        ...
                        # Creates the popup, with the FLoatLayout as the content, and moves it to a location the user
                        # will not see, so the popup will not be visable, because the popup does not look
                        # aesthetically pleasing
                        popup = Popup(content=float, size_hint=(None, None), size=(120, 160), auto_dismiss=False,
                                      pos_hint={'x': 10.0, 'y': 10.0})
    ...
    def check_for_promotion(self):
        ...
        if str(self.pos_chess) == "a8" or str(self.pos_chess) == "b8" or str(self.pos_chess) == "c8" or str(
                self.pos_chess) == "d8" or str(self.pos_chess) == "e8" or str(self.pos_chess) == "f8" or str(
            self.pos_chess) == "g8" or str(self.pos_chess) == "h8":
            if str(self.piece_that_moved)[6] == 'P':
                ...
                or str(self.pos_chess) == "d1" or str(self.pos_chess) == "e1" or str(self.pos_chess) == "f1" \
                or str(self.pos_chess) == "g1" or str(self.pos_chess) == "h1":
            if str(self.piece_that_moved)[6] == 'P':
    ...
    def promotion(self, obj):
        ...
        move = chess.Move(number_conversion[str(self.chess_position_numerical)] - 1,
                          number_conversion[str(self.pos_chess)] - 1, promotion=self.promotion_number)
        self.board.push(move)
        self.move_worked = True
        self.turn += 1
    ...
    def every_move_functionality(self, conversion, piece_that_moved):
        self.ids[piece_that_moved].pos = (
            conversion.to_number()[self.pos_chess][0], conversion.to_number()[self.pos_chess][1])
        ...
        self.ids["Trail One"].pos = (
            conversion.to_number()[self.pos_chess][0], conversion.to_number()[self.pos_chess][1])
        self.ids["Trail Two"].pos = (conversion.to_number()[self.chess_position_numerical][0],
                                     conversion.to_number()[self.chess_position_numerical][1])
    ...
    def agent_move(self, conversion):
        if self.board.is_game_over():
            ...
            btn = Button(text='Exit', size_hint=(.2, .2), pos_hint={'center_x': .7, 'center_y': .5})
            btn.bind(on_press=lambda obj: Window.close())
            popup = Popup(title='Congratulation, You won!', content=btn,
        ...
        action = self.player(self.game.getCanonicalForm(self.board, current_player))
        valids = self.game.getValidMoves(self.game.getCanonicalForm(self.board, current_player), 1)

        if valids[action] == 0:
            print(action)
            assert valids[action] > 0
        move = self.game.make_move(self.board, current_player, action)
        ...
        self.ids[position_dic[str(move)[0] + str(move)[1]]].pos = (
            conversion.to_number()[str(move)[2] + str(move)[3]][0],
            conversion.to_number()[str(move)[2] + str(move)[3]][1])
        ...
        self.ids["Trail Two"].pos = (
            conversion.to_number()[str(move)[0] + str(move)[1]][0],
            conversion.to_number()[str(move)[0] + str(move)[1]][1])
